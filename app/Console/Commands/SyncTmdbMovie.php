...
class SyncTmdbMovie extends Command
{
    ...
    if (!empty($category) && empty($categoryId)) {
        // Try an exact slug match using the provided --category value.
        // If found, treat it as a category_id so later persistence will associate via relation.
        $existing = Category::where('slug', (string) $category)->first();
        if ($existing) {
            $categoryId = $existing->id;
            $category = null;
            $io->text(sprintf('Resolved --category to existing Category id=%d (slug="%s").', $categoryId, $existing->slug ?? ''));
        }
    }
    ...
    if (!empty($categoryId) || !empty($category)) {
        if (Schema::hasColumn('movies', 'category_id')) {
            ...
            if (!$target && !empty($category)) {
                $target = Category::where('slug', (string) $category)->first();
            }
